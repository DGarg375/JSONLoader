...
import { deg2rad, loadExternalFile } from './js/utils/utils.js'
...
class OBJLoader {
     ...
     * 
     * @returns {[Array<Number>, Array<Number>, Material]} A triple / list containing 1) the list of vertices and 2) the list of triangle indices and 3) a material
     */
    load() {
    ...
    parseVec3(vec_string) {
        ...
        let vertex_list = vec_string.split(' ');
        return [parseFloat(vertex_list[1]), parseFloat(vertex_list[2]), parseFloat(vertex_list[3])];
    ...
    parseFace(face_string, entry_index)
    {
        ...
        for(let i of face_string.split(' ')) {
            result.push(parseInt(i.split('/')[entry_index])-1)
        }

        result.shift();

        if(result.length == 4) {
            result = this.triangulateFace(result);
        }
